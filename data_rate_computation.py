...
for x in mycol.find():
  ...
  """print('Length of iq sample: ',len(x.get('event').get('iq')))
  print('Length of iq sample (bytes): ',len(x.get('event').get('iq').encode('utf-8')))
  print('Total doc length: ',len(bson.BSON.encode(x)))
  break"""
